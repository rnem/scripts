disable-local-users:
    cmd.run:
        - name: |
            ...
            ##  v0006 - UB: Fixed the logic to successfull dedect if users were found.     ##
            ##              Added OS detection, for Ubuntu, no actions done anymore.       ##
            ##  v0007 - RN: Fixed empty User -Info, -Home and -Shell, that these values    ##
            ...
            ##  v0009 - RN: Add Clear names for the L/LK, P/PS, NP status for the PWStatus ##
            ##              Put the FQP to all tools I have used.                          ##
            ##  v0010 - RN: Add sectransfer ignore for Security Team on PMA server         ##
            ##              Add vm-all-dsu-test to the server exclude list (DSU)           ##
            ##              Add vm-all-all-Git-server1 to the server exclude list (DSU)    ##
            ##              Make the servername compare more reliable                      ##
            ##  v0011 - UB: Add some more servers to the exclusion list to skip these ones.##
            ##  v0012 - RN: Agree with Roger to activate the script, so it will disable all##
            ##              local users now.                                               ##
            ##  v0013 - UB: Added /usr/bin/scponly for a whitelisted user shell.           ##
            ...
            ##  v0015 - RN: Changed the Text for Excluded servers from "Excluded" to       ##
            ##        "No action taken".                         			               ##
            #@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            # DEBUG Mode means not disable any user. false=DEBUG off!
			DEBUG="false"
			# User ArraHallo auch ...y
            USERARR=()
            CSVHEADER="Hostname,Username,ShadowUser,UserID,UserInfo,UserHome,UserShell,UserPWStatus,LastPWChange,Status,ScriptRunAt,OSBrand,DryRun"
            # Hostname of the current system
			HOSTNAME=$(/bin/hostname)
            # No local users = true otherwise false
			NOLOCALUSERS="true"
            ...
            # Exclude this server: true=yes, false=no (default)
			EXCLUDESRV="false"
            # Array with servers to be excluded by this script, e.g. cPanel or other reason
            SRVEXCLarr=( "vm-all-dsu-test"
            "vm-all-all-git-servers"
            "vm-ams-cl-domainfelixlacom-prod"
            "vm-emea-it-domainpurone-production"
            "vm-apac-au-domainone-production"
            )
            ...
            for i in $( egrep -v "/sbin/nologin|/bin/false|/usr/bin/scponly" /etc/passwd | awk -F: '{print $1}' | sort | /bin/egrep -v "root|cloud|mysql|cloudconnect|centos|clamav|clam|cloud-user|postgres|synchronize|dsuser|svn|davigelcron|castelis|git|gitlab|solr|sync|shutdown|halt|monit|apache|cron_user|cronjob|tomcat|speech-dispatcher|app_syncuser|sectransfer|cron_sectransfer|fedora|jenkins|munin" )
            do
            USERARR+=( ${i} );
            done
            ...
            echo ${CSVHEADER}
            # Collect details about the found users
