#!/bin/bash
##################################################################
# RED HAT SATELLLITE - Promote latest NonProd CCVs to Prod       #
...
declare -a all_envs
all_envs[0]='ccv-org1-standard;External-Production-DMZ'
all_envs[1]='ccv-org1-standard;Internal-Production'
...
for envs in "${all_envs[@]}"; do
  IFS=";" read -r -a arr <<< "${envs}"
    ...
    hammer content-view version promote --organization-label ${ORG_LABEL} --content-view "${arr[0]}" --to-lifecycle-environment "${arr[1]}" --version "${latest_version}" --description "promoted latest NonProd to Prod LCE"
