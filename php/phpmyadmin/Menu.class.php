...
class PMA_Menu
{
    ...
    /**
     * Silke Suck: Password change information
     *
     * @access private
     * @var string
     */
    private $_pwchangeinfo;
    ...
    private function _getBreadcrumbs()
    {
        ...
        $retval .= "<br />";
        $this->_getPwChangeInfo();
        $retval .= $this->_getPwChangeInfo();
        $retval .= '</div>';
        return $retval;
    }
    ...
    # =================================================
    # Function for the Last Password Change Information
    # =================================================
    private function _getPwChangeInfo()
    {
      if (isset($_GLOBALS['cfg']['Server']['lastchange']))
      {
        $lastchange = $_GLOBALS['cfg']['Server']['lastchange'];
      }
      else
      {
        require '/etc/phpMyAdmin/pwtrack.inc.php';
        $pwserver = $GLOBALS['cfg']['Server']['host'];
        //echo "pwserver: $pwserver";

        $db = new mysqli($pwserver, $pwtrackuser, $pwtrackpass, $pwtrackdb);

        if (!($prep1 = $db->prepare("SELECT UNIX_TIMESTAMP(lastchange) FROM pwtrack WHERE user = ? AND host = ?")))
        {
          return;
        }
        $prep1->bind_param('ss', $pwuser, $pwhost);

        $pwuser   = $GLOBALS['cfg']['Server']['user'];
        $pwhost   = $GLOBALS['_SERVER']['SERVER_ADDR'];
        //echo "SELECT UNIX_TIMESTAMP(lastchange) FROM pwtrack WHERE user = '$pwuser' AND host = '$pwhost'";
        //echo "pwuser: $pwuser - pwhost: $pwhost";

        $prep1->execute();
        $res1 = $prep1->get_result();
        $row1 = $res1->fetch_all();
        if (count($row1) < 1)
        {
          return '<span style="color:white;background-color:red">Last password change could not be determined, please contact DSU.</span>';
        }

        $lastchange = $row1[0][0];
        $_GLOBALS['cfg']['Server']['lastchange'] = $lastchange;
      }
      $day      = 60 * 60 * 24;
      $changediff = time() - $lastchange;
      $diffdays = floor($changediff / $day);

      $retval = '<span style="color:';
      if ($changediff >= ($day * 85))
      {
        $retval .= 'white;background-color:red';
      }
      else if ($changediff >= ($day * 80))
      {
        $retval .= 'yellow';
      }
      else if ($changediff < ($day * 30))
      {
        $retval .= 'chartreuse';
      }
      else
      ...
      $retval .= '">Your last password change was ' . $diffdays
       . ' day(s) ago.';
      if ($changediff >= (90 * $day))
      {
        $retval .= ' Please change your password immediately!';
      }
      else if ($changediff >= (80 * $day))
      {
        $retval .= ' Please change your password within the next '
          . (90 - $diffdays) . ' days.';
      }
      $retval .= '</span>';
      return $retval;
